...
# originally by Mick Douglas @BetterSafetyNet - https://github.com/besimorhino/Pause-Process
# forked from https://github.com/stahler/Pause-Process
# This fork by Michael Woods @TheWoodsy - https://github.com/TheWoodsy/Process-Pause
...
.SYNOPSIS
This is a PowerShell script which allows one to Process-Pause, Process-UnPause or -Process-TogglePause.

.DESCRIPTION
This script will allow users to pause, unpause and toggle running commands.  This is
...
.EXAMPLE
Process-Pause -ID [PID]

.EXAMPLE
Process-UnPause -ID [PID]

.EXAMPLE
Process-TogglePause -ID [PID]
...
.LINK
https://github.com/TheWoodsy/Process-Pause
...
function Test-DebuggerPresent {
    ...
    Begin {
        Write-Host ("Checking if debugger is present for PID: $ID")
    }
    ...
    End {
        if ($isDebuggerPresent -eq $True) {
            Write-Host ("Debugger exists for process $ID")
            $True
        }
        else {
            Write-Host ("Debugger does not exist for $ID")
            $False
        }
    }
}
function Process-Pause {
    ...
    Begin {
        ...
        Write-Host ("you entered an ID of: $ID")
    }
    ...
    End {
        if ($PauseResult -eq $False) {
            ...
            Write-Host ("Process $ID was paused")
        }

    }
}

function Process-UnPause {
    [CmdletBinding()]
    Param (
        [parameter(Mandatory = $True, ValueFromPipelineByPropertyName = $True)]
        [alias("OwningProcess")]
    ...
    Begin {
        Write-Host ("Attempting to unpause PID: $ID")
    }
    ...
    Process {
        ...
        if ((Test-DebuggerPresent -ID $id) -eq $False) {
            ...
            Write-Warning ("Process is not currently being debugged")
        }
        else {
...
function Process-TogglePause {
    ...
    Begin {
        Write-Host ("Attempting to toggle PID: $ID")
    }

    Process {
        # Attempt the toggle
        if ((Test-DebuggerPresent -ID $id) -eq $False) {
            $TogglePauseResult -eq $False
            Write-Host -("Process is not currently being debugged - pausing")
            $TogglePauseResult = [NativeMethods]::DebugActiveProcess($ID)
        }
        else {
            Write-Host ("Process is being debugged - unpausing")
            $TogglePauseResult = [NativeMethods]::DebugActiveProcessStop($ID)
        }
    }

    End {
        if ($TogglePauseResult -eq $False) {
            Write-Error ("unable to unpause process $ID. Is it running or gone?")
        }
        else {
            Write-Host ("$ID was toggled")
        }
    }
}
